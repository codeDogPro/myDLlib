...
namespace dl {
  ...
  conv2D_cuda(thrust::device_ptr<T> input, thrust::device_ptr<T> output,
              ...
              int k_size, int stride, int irow, int icol, int ichannel, int noffset){
    ...
    int koffset = blockIdx.z * k_size*k_size*ichannel;
    int ooffset = noffset/(irow*icol*ichannel) * (orow*ocol*blockDim.z);
    int oidx = ooffset + idx_y*ocol + idx_x;

    // conv all channel
    for(int ch = 0; ch < ichannel; ch++){
      int ioffset = noffset + ch * irow*icol;
      koffset += ch * k_size*k_size;
