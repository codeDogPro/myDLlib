...
namespace dl{
  ...
  (thrust::device_ptr<const T> input, thrust::device_ptr<T> output, int n, int square){
    __shared__ T s_input[128];
    const int by = blockIdx.y, bx = blockIdx.x, tx = threadIdx.x;
    const int idx = bx * blockDim.x + tx;

    s_input[tx] = (idx < n) ? input[by * square + idx] : static_cast<T>(0); 
    __syncthreads();
    ...
    for(int ofst = 16; ofst > 0; ofst >>= 1){
      value += __shfl_down_sync(-1, value, ofst);
    }
