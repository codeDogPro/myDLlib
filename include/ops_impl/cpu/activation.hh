...
template <typename T>
bool relu_cpu(int task_begin, int task_num, int shape, int offset,
                   std::shared_ptr<Tensor<T>> output,
                   const std::shared_ptr<const Tensor<T>> input) {
  int start = offset + shape * task_begin, end = start + shape * task_num;
  for (int i = start; i < end; i++) {
    (*output)[i] =
        (*input)[i] > static_cast<T>(0) ? (*input)[i] : static_cast<T>(0);
  }
  return true;
}

template <typename T>
bool sigmoid_cpu(int task_begin, int task_num, int shape, int offset,
                 std::shared_ptr<Tensor<T>> output,
                 const std::shared_ptr<const Tensor<T>> input) {
  int start = offset + shape * task_begin, end = start + shape * task_num;
  for (int i = start; i < end; i++) {
  ...
  template<typename T>
  bool exp_cpu (int task_begin, int task_num, int shape, int offset,
                     std::shared_ptr<Tensor<T>> output,
                     const std::shared_ptr<const Tensor<T>> input){ 
    int start = offset + shape * task_begin, end = start + shape * task_num;
    for(int i = start; i < end; i++){
  ...
  template<typename T>
  bool softmax_axis0_cpu(int task_begin, int task_num, int shape, int offset,
                              std::shared_ptr<Tensor<T>> output, 
                              T *input){ 
    int start = offset + shape * task_begin, end = start + shape * task_num;
  ...
  template<typename T>
  bool softmax_axis1_cpu(int task_begin, int task_num, int shape, int offset,
                              std::shared_ptr<Tensor<T>> output, 
                              T *input){ 
    int start = offset + shape * task_begin, end = start + shape * task_num;
    int row = output->row(), col = output->col();
  ...
  template<typename T>
  bool softmax_axis2_cpu(int task_begin, int task_num, int shape, int offset,
                              std::shared_ptr<Tensor<T>> output, 
                              T *input){ 
    int row = output->row(), col = output->col(), channel = output->channel();
